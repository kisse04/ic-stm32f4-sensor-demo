...
const char *IC_Status_CategoryString(ic_status_t status)
{
    ...
    else if (IC_STATUS_IS_BOARD_ERR(status))
    {
        return "Board";
    }
    else if (IC_STATUS_IS_BUS_ERR(status))
    {
        return "Bus";
    }
    else if (IC_STATUS_IS_LED_ERR(status))
    {
        return "LED";
    }
    else if (IC_STATUS_IS_BMP280_ERR(status))
    {
        return "BMP280";
    }
    else if (IC_STATUS_IS_VL53L0X_ERR(status))
    {
        return "VL53L0X";
    }
    else
