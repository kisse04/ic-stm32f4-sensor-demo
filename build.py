...
def main() -> int:
    parser = argparse.ArgumentParser(description="GCC build script (ported from .bat)")
    ...
    # Env: set PATH=%GCC_BIN%;%PATH%  (keep it, though we won't rely on it)
    env = os.environ.copy()
    ...
    startup_o = out_dir / "startup_stm32f446retx.o"
    ...
    with rsp.open("w", encoding="utf-8") as f:
        for obj in objects:
            f.write(windows_path_to_posix(obj.resolve()) + "\n")
    ...
    cmd = [str(gcc_exe), *cflags, f"@{str(rsp)}", *ldflags, "-o", str(elf)]
    rc = run_cmd(log_path, cmd, env=env)
    if rc != 0:
